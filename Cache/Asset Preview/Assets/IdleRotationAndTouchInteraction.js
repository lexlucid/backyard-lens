...
script.createEvent('TouchStartEvent').bind(function(eventData) {
    ...
    touches.push(eventData.getTouchId());
...
script.createEvent('TouchMoveEvent').bind(function(eventData) {
    ...
    if (touches.length === 2 && eventData.getTouchId() === touches[1]) {
        secondTouchPos = eventData.getTouchPosition();
        updateDistance(touchPos.distance(secondTouchPos));
    }
